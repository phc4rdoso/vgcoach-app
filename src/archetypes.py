def determine_archetypes(team_data, all_moves):
    ...
    if has_weather:
        archetypes.append(("???", "Weather", "Relies on weather conditions to boost attacks or trigger abilities."))
    ...
    if team_data:
        ...
        avg_bulk = sum(p.get('HP', 100) + p.get('Def', 100) + p.get('SpD', 100) for p in team_data) / len(team_data)
    else:
        avg_speed = 100
        avg_bulk = 300
    ...
    if recovery_count >= 5 and avg_bulk > 280:
    ...
    if avg_speed > 95 and avg_bulk < 260:
        archetypes.append(("??", "Hyper Offense", "Prioritizes speed and damage to overwhelm the opponent quickly."))
        
    # 6. Bulky Offense
    elif avg_bulk > 260 and avg_speed < 90 and "Trick Room" not in all_moves:
        archetypes.append(("???", "Bulky Offense", "Uses naturally bulky attackers that can take hits and hit back hard."))
        
    # 7. Balance
    elif ("Fake Out" in all_moves or "Incineroar" in pokemon_names) and avg_bulk >= 230:
        archetypes.append(("??", "Balance", "A flexible composition with a mix of offense, defense, and support."))
